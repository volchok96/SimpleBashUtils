...
int main(int argc, char *argv[]) {
  if (argc == 1) {
    printf("usage: grep [-eivclnhsfo] [file ...] [-e pattern] [-f file]\n");
    exit(EXIT_FAILURE);
  }
  flags_grep flag = {0};
  check_flags_grep(argc, argv, &flag);
  open_file(argc, (const char **)argv, &flag);
  return 0;
}

struct option long_options[] = {{"e", no_argument, 0, 'e'},
...
void check_flags_grep(int argc, char **argv, flags_grep *flag) {
  int res = 0;
  opterr = 0;
  while ((res = getopt_long(argc, argv, "e:ivclnhsf:o", long_options, 0)) !=
         -1) {
    switch (res) {
    case 'e':
      flag->e = 1;
    ...
    case '?':
      dprintf(2, "%s: invalid option -- %s\n", argv[0] + 2,
              ((argv[optind - 1]) + 1));
      dprintf(2, "usage: %s [-eivclnhsfo] [file ...] [-e pattern] [-f file] \n",
              argv[0] + 2);
      exit(1);
    }
  }
...
void search_and_print(const char *argv[], FILE *fd, flags_grep *flag,
  ...
  if (regcomp(&rx, flag->search_express, cflags) != 0)
    exit(EXIT_FAILURE);
